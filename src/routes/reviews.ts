...
export function registerReviewRoutes(app: Hono<AppEnv>) {
  ...
  app.get('/v1/reviews/:reviewId', async (c) => {
    ...
    return c.json({ review, draft, actions });
  });
  ...
  app.post('/v1/reviews/:reviewId/process', async (c) => {
    ...
    return c.json(await new ReviewService(c.get('repository'), c.get('intelligence'), c.get('actor')).process(c.req.param('reviewId')));
  });
  app.post('/v1/reviews/:reviewId/qa', async (c) => {
    await requireReviewRole(c.get('repository'), c.req.param('reviewId'), c.get('actor'), ['buyer', 'approver', 'operator']);
    const input = parse(qaSchema, await jsonBody(c));
    return c.json(await new ReviewService(c.get('repository'), c.get('intelligence'), c.get('actor')).qa(c.req.param('reviewId'), input.confirmedActions));
  });
  app.post('/v1/reviews/:reviewId/decision', async (c) => {
    ...
    return c.json(await new ReviewService(c.get('repository'), c.get('intelligence'), c.get('actor')).decide(c.req.param('reviewId'), input));
  });
  app.post('/v1/reviews/:reviewId/publication', async (c) => {
    await requireReviewRole(c.get('repository'), c.req.param('reviewId'), c.get('actor'), ['buyer', 'approver', 'operator']);
    const input = parse(publicationSchema, await jsonBody(c));
    const review = await new ReviewService(c.get('repository'), c.get('intelligence'), c.get('actor')).confirmPublication(c.req.param('reviewId'), input.confirmed, { evidence: input.evidence, reasonNotPublished: input.reasonNotPublished });
  ...
  app.post('/v1/reviews/:reviewId/escalate', async (c) => {
    ...
    return c.json({ review: await new ReviewService(c.get('repository'), c.get('intelligence'), c.get('actor')).escalate(c.req.param('reviewId'), body.reason) });
  });
