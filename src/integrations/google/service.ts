...
import { SupabaseGoogleIntegrationStore, type GoogleIntegrationStore } from './store';
import type { GoogleAccount, GoogleConnection, GoogleReview, GoogleReviewLink, GoogleSyncRun } from './types';
...
function addSeconds(seconds: number): string { return new Date(Date.now() + Math.max(30, seconds) * 1000).toISOString(); }
...
function connectionView(connection: GoogleConnection): GoogleConnectionView {
  const { accessTokenCiphertext: _access, refreshTokenCiphertext: _refresh, tokenType: _type, ...view } = connection;
  return view;
}
...
export class GoogleIntegrationService {
  ...
  async startAuthorization(actor: Actor, restaurantId: string) {
    ...
    await this.store.createOAuthFlow({
      id: newId(), restaurantId, actorId: actor.id,
    ...
    return { authorizationUrl: this.client.buildAuthorizationUrl(state, challenge), expiresAt: new Date(now.getTime() + 10 * 60 * 1000).toISOString() };
  }
  ...
  async completeAuthorization(input: { code?: string; state?: string; error?: string }) {
    ...
    const connection = await this.store.upsertConnection({
      id: existing?.id ?? newId(), restaurantId: flow.restaurantId, status: 'connected',
      accessTokenCiphertext: await this.cipher.seal(tokens.access_token), refreshTokenCiphertext,
      tokenType: tokens.token_type ?? 'Bearer', scope: tokens.scope ?? 'https://www.googleapis.com/auth/business.manage',
      expiresAt: addSeconds(tokens.expires_in), externalAccountName: existing?.externalAccountName,
      externalAccountDisplayName: existing?.externalAccountDisplayName,
      selectedLocationName: existing?.selectedLocationName, selectedLocationTitle: existing?.selectedLocationTitle,
      connectedBy: flow.actorId, connectedAt, updatedAt: now,
    });
  ...
  async selectAccount(restaurantId: string, accountName: string) {
    ...
    const updated = await this.store.updateConnection(restaurantId, {
      ...
      selectedLocationTitle: undefined,
  ...
  async refreshLocations(restaurantId: string) {
    ...
    return this.store.replaceLocations(restaurantId, connection.id, locations.map((location) => ({
      name: location.name, title: location.title ?? location.name, storeCode: location.storeCode, metadata: location.metadata,
    })));
  }

  async listStoredLocations(restaurantId: string) { return this.store.listLocations(restaurantId); }
  ...
  async selectLocation(restaurantId: string, locationName: string) {
    ...
    if (!locations.some((location) => location.name === locationName)) throw new AppError('The selected Google location is not accessible.', 422, 'google_location_invalid');
    const selected = await this.store.selectLocation(restaurantId, locationName);
    await this.store.updateConnection(restaurantId, {
      selectedLocationName: selected.name, selectedLocationTitle: selected.title,
      updatedAt: new Date().toISOString(), lastError: undefined,
    });
    return selected;
  }
  ...
  async syncReviews(actor: Actor, repository: Repository, restaurantId: string) {
    const connection = await this.requiredConnection(restaurantId);
    if (!connection.externalAccountName || !connection.selectedLocationName) throw new AppError('Select a Google account and location before syncing.', 409, 'google_location_required');
    ...
    try {
      const result = await this.client.listReviews(await this.accessToken(restaurantId), connection.externalAccountName, connection.selectedLocationName);
      run.pagesFetched = result.pagesFetched; run.reviewsSeen = result.reviews.length;
      for (const googleReview of result.reviews) {
        ...
        if (link && link.googleUpdateTime === googleReview.updateTime) {
          ...
          await this.store.upsertReviewLink({ ...link, contentExpiresAt: new Date(Date.now() + thirtyDaysMs).toISOString(), updatedAt: new Date().toISOString() });
          continue;
        }
        ...
        if (localReview) {
          localReview = await repository.updateReview(localReview.id, {
            rating: ratingMap[googleReview.starRating], reviewDate: googleReview.createTime.slice(0, 10),
            reviewerDisplayName: googleReview.reviewer?.displayName, originalText: googleReviewText(googleReview),
            sourceReference: googleReview.name, classification: undefined, state: 'verified', updatedAt: new Date().toISOString(),
          }, actor);
          run.reviewsUpdated += 1;
        } else {
          const now = new Date().toISOString();
          const input: Review = {
            id: newId(), restaurantId, source: 'google', sourceReference: googleReview.name,
            rating: ratingMap[googleReview.starRating], reviewDate: googleReview.createTime.slice(0, 10),
            reviewerDisplayName: googleReview.reviewer?.displayName, originalText: googleReviewText(googleReview),
            serviceMode: 'unknown', ingestionMethod: 'api', verificationStatus: 'verified', state: 'verified',
            createdBy: actor.id, createdAt: now, updatedAt: now,
          };
          localReview = await repository.createReview(input, actor); run.reviewsImported += 1;
        }
        const now = new Date().toISOString();
        await this.store.upsertReviewLink({
          id: link?.id ?? newId(), restaurantId, connectionId: connection.id,
          googleReviewName: googleReview.name, googleReviewId: googleReview.reviewId,
          localReviewId: localReview.id, googleUpdateTime: googleReview.updateTime, googleCreateTime: googleReview.createTime,
          replyComment: googleReview.reviewReply?.comment, replyUpdateTime: googleReview.reviewReply?.updateTime,
          contentExpiresAt: new Date(Date.now() + thirtyDaysMs).toISOString(), createdAt: link?.createdAt ?? now, updatedAt: now,
        });
      }
      return this.store.updateSyncRun(run.id, { ...run, status: 'succeeded', completedAt: new Date().toISOString() });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown Google sync error.';
      await this.store.updateConnection(restaurantId, { status: 'error', lastError: message, updatedAt: new Date().toISOString() }).catch(() => undefined);
      await this.store.updateSyncRun(run.id, { ...run, status: 'failed', error: message, completedAt: new Date().toISOString() });
      throw error;
    }
  }

  async publishApprovedReply(actor: Actor, repository: Repository, reviewId: string, consent: boolean) {
    if (!this.config.replyWritesEnabled) throw new AppError('Google reply writes are disabled for this environment.', 503, 'google_reply_writes_disabled');
    ...
    if (!['approved', 'edited'].includes(review.state)) throw new AppError('Only an approved review reply can be published.', 409, 'google_reply_not_approved');
    ...
    if (!draft || !comment || draft.status !== 'approved') throw new AppError('The latest draft is not approved.', 409, 'google_reply_not_approved');
    ...
    if (!connection.externalAccountName || !connection.selectedLocationName) throw new AppError('Google location selection is incomplete.', 409, 'google_location_required');
    const reply = await this.client.updateReply(await this.accessToken(review.restaurantId), connection.externalAccountName, connection.selectedLocationName, link.googleReviewId, comment);
    await this.store.upsertReviewLink({ ...link, replyComment: reply.comment ?? comment, replyUpdateTime: reply.updateTime ?? new Date().toISOString(), updatedAt: new Date().toISOString() });
    const updatedReview = await repository.updateReview(review.id, { state: 'published', updatedAt: new Date().toISOString() }, actor);
  ...
  async disconnect(restaurantId: string, revoke: boolean) {
    ...
    return connectionView(await this.store.updateConnection(restaurantId, {
      status: 'disconnected', accessTokenCiphertext: '', refreshTokenCiphertext: '',
      expiresAt: new Date(0).toISOString(), disconnectedAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    }));
  }

  async purgeExpired(actor: Actor, repository: Repository, limit = 100) {
    const links = await this.store.listExpiredReviewLinks(new Date().toISOString(), Math.min(Math.max(limit, 1), 500));
    let deleted = 0; const failures: Array<{ linkId: string; error: string }> = [];
    for (const link of links) {
      try { await repository.deleteReview(link.localReviewId, actor); await this.store.deleteReviewLink(link.id); deleted += 1; }
      catch (error) { failures.push({ linkId: link.id, error: error instanceof Error ? error.message : 'Unknown deletion error.' }); }
    }
    return { considered: links.length, deleted, failures };
  }

  async listSyncRuns(restaurantId: string) { return this.store.listSyncRuns(restaurantId); }

  private assertEnabled() {
    if (!this.config.enabled) throw new AppError('Google integration is disabled for this environment.', 503, 'google_integration_disabled');
  }
  private async requiredConnection(restaurantId: string) {
    this.assertEnabled(); const connection = await this.store.getConnection(restaurantId);
    if (!connection || connection.status === 'disconnected') throw new AppError('Google is not connected for this restaurant.', 409, 'google_not_connected');
    return connection;
  }
  private async accessToken(restaurantId: string): Promise<string> {
    const connection = await this.requiredConnection(restaurantId);
    if (new Date(connection.expiresAt).getTime() > Date.now() + 60_000 && connection.accessTokenCiphertext) return this.cipher.open(connection.accessTokenCiphertext);
    if (!connection.refreshTokenCiphertext) {
      await this.store.updateConnection(restaurantId, { status: 'needs_reauth', lastError: 'Refresh token missing.', updatedAt: new Date().toISOString() });
    ...
    try {
      ...
      const updated = await this.store.updateConnection(restaurantId, {
        status: 'connected', accessTokenCiphertext: await this.cipher.seal(refreshed.access_token),
        expiresAt: addSeconds(refreshed.expires_in), scope: refreshed.scope ?? connection.scope,
        tokenType: refreshed.token_type ?? connection.tokenType, lastError: undefined, updatedAt: new Date().toISOString(),
      });
      return this.cipher.open(updated.accessTokenCiphertext);
    } catch (error) {
      await this.store.updateConnection(restaurantId, { status: 'needs_reauth', lastError: error instanceof Error ? error.message : 'Token refresh failed.', updatedAt: new Date().toISOString() });
      throw error;
    }
  }
}
...
export function googleIntegrationForEnv(env: CloudflareBindings): GoogleIntegrationService {
  const config: GoogleIntegrationConfig = {
    ...
    redirectUri: env.GOOGLE_REDIRECT_URI ?? '', successUrl: env.GOOGLE_OAUTH_SUCCESS_URL,
  };
  if (config.enabled && (!config.clientId || !config.clientSecret || !config.redirectUri || !env.GOOGLE_TOKEN_ENCRYPTION_KEY || !env.SUPABASE_SERVICE_ROLE_KEY)) {
