...
export interface GoogleIntegrationStore {
  ...
  listExpiredReviewLinks(restaurantId: string, now: string, limit: number): Promise<GoogleReviewLink[]>;
...
export class MemoryGoogleIntegrationStore implements GoogleIntegrationStore {
  ...
  async updateConnection(restaurantId: string, patch: Partial<GoogleConnection>) {
    ...
    if (!existing) throw new AppError('Google connection not found.', 404, 'not_found');
    const updated = { ...existing, ...patch };
    this.connections.set(restaurantId, clone(updated));
  ...
  async replaceLocations(restaurantId: string, connectionId: string, candidates: GoogleLocationCandidate[]) {
    ...
    for (const row of rows) this.locations.set(row.id, clone(row));
    return clone(rows);
  }
  async listLocations(restaurantId: string) {
    return clone([...this.locations.values()].filter((item) => item.restaurantId === restaurantId));
  }
  async selectLocation(restaurantId: string, locationName: string) {
    ...
    if (!selected) throw new AppError('Google location not found.', 404, 'not_found');
  ...
  async updateSyncRun(id: string, patch: Partial<GoogleSyncRun>) {
    ...
    if (!existing) throw new AppError('Sync run not found.', 404, 'not_found');
    const updated = { ...existing, ...patch };
    this.syncRuns.set(id, updated);
    return clone(updated);
  }
  async listSyncRuns(restaurantId: string) {
    return clone([...this.syncRuns.values()].filter((run) => run.restaurantId === restaurantId));
  }
  ...
  async findReviewLinkByLocalReviewId(localReviewId: string) {
    return clone([...this.links.values()].find((link) => link.localReviewId === localReviewId) ?? null);
  }
  async upsertReviewLink(link: GoogleReviewLink) {
    ...
    this.links.set(value.id, clone(value));
    return clone(value);
  }
  async listExpiredReviewLinks(restaurantId: string, now: string, limit: number) {
    return clone([...this.links.values()]
      .filter((link) => link.restaurantId === restaurantId && link.contentExpiresAt <= now)
      .slice(0, limit));
  }
...
export class SupabaseGoogleIntegrationStore implements GoogleIntegrationStore {
  ...
  async consumeOAuthFlow(stateHash: string, now: string) {
    const { data, error } = await this.client.from('google_oauth_flows')
  ...
  async upsertConnection(connection: GoogleConnection) {
    const { data, error } = await this.client.from('google_connections')
      .upsert(toSnake(connection), { onConflict: 'restaurant_id' })
      .select()
      .single();
  ...
  async updateConnection(restaurantId: string, patch: Partial<GoogleConnection>) {
    ...
    delete mutable.id; delete mutable.restaurantId;
    const { data, error } = await this.client.from('google_connections')
      .update(toSnake(mutable))
  ...
  async replaceLocations(restaurantId: string, connectionId: string, locations: GoogleLocationCandidate[]) {
    ...
    const now = new Date().toISOString();
    const rows = locations.map((location) => toSnake({
      ...location,
      restaurantId,
      connectionId,
      selected: false,
  ...
  async listLocations(restaurantId: string) {
    const { data, error } = await this.client.from('google_location_candidates')
      .select('*')
      .eq('restaurant_id', restaurantId)
      .order('title');
    if (error) throw new AppError(error.message, 500, 'google_store_error');
  ...
  async selectLocation(restaurantId: string, locationName: string) {
    const { error: resetError } = await this.client.from('google_location_candidates')
  ...
  async updateSyncRun(id: string, patch: Partial<GoogleSyncRun>) {
    const mutable = { ...patch } as Record<string, unknown>;
    delete mutable.id;
    const { data, error } = await this.client.from('google_sync_runs')
      .update(toSnake(mutable))
      .eq('id', id)
      .select()
      .single();
  ...
  async findReviewLinkByGoogleName(restaurantId: string, googleReviewName: string) {
    const { data, error } = await this.client.from('google_review_links')
      .select('*')
      .eq('restaurant_id', restaurantId)
  ...
  async findReviewLinkByLocalReviewId(localReviewId: string) {
    const { data, error } = await this.client.from('google_review_links')
      .select('*')
      .eq('local_review_id', localReviewId)
      .maybeSingle();
    if (error) throw new AppError(error.message, 500, 'google_store_error');
    return data ? (toCamel(data) as unknown as GoogleReviewLink) : null;
  }
  async upsertReviewLink(link: GoogleReviewLink) {
    const { data, error } = await this.client.from('google_review_links')
      .upsert(toSnake(link), { onConflict: 'restaurant_id,google_review_name' })
      .select()
      .single();
